...
const deviceTime = (timestamp: string) => {
  ...
  // The attendance device time is already local time.
  // Read the HH:mm directly instead of converting UTC → local.
...
async function getDeviceAttendanceByDate(date: string): Promise<DeviceAttendanceLog[]> {
  ...
  if (!response.ok) {
    throw new Error(`Face-recognition API returned ${response.status}`);
  }
  ...
  if (!Array.isArray(logs)) {
    throw new Error("Invalid face-recognition attendance response");
  }
...
function deviceLogsToEntries(
  ...
  for (const employee of employees) {
    ...
    const employeeLogs = logs
      .filter((log) => {
        const deviceId = String(log.deviceUserId)
          .replace(/[\u0000-\u001F\u007F]/g, "")
          .trim();
...
export default function AttendancePage({
  ...
  const [deviceLogs, setDeviceLogs] = useState<DeviceAttendanceLog[]>([]);
...
setSpecialTimes(special);
  ...
  // ── Load worktime + face-recognition attendance for the selected date ─────
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDeviceAttendanceByDate(currentDate),
    ])
     .then(([records, logs]) => {
  if (cancelled) return;

  console.log("=================================");
  console.log("ATTENDANCE DATE:", currentDate);
  console.log("EMPLOYEES:", employees);
  console.log("DEVICE LOGS:", logs);

  const employee104 = employees.find(
    (e) => String(e.empNumber) === "104"
  );

  console.log("EMPLOYEE 104:", employee104);

  const logs104 = logs.filter(
    (log) => String(log.deviceUserId).trim() === "104"
  );

  console.log("LOGS FOR EMPLOYEE 104:", logs104);

  setDeviceLogs(logs);

  const machineEntries = deviceLogsToEntries(logs, employees);

  console.log("MACHINE ENTRIES:", machineEntries);
  console.log("MACHINE ENTRY 104:", machineEntries["104"]);

  setDeviceEntries(machineEntries);

        setEntries((prev) => {
          const fromStorage = loadEntriesFromStorage(currentDate);
          const next = { ...fromStorage };

          records.forEach((r: WorktimeRecord) => {
          ...
          // Face-recognition data is authoritative for clock-in/out.
          employees.forEach((emp) => {
            const machine = machineEntries[emp.num];
            if (!machine) return;

            (assignedShifts[emp.num] ?? []).forEach((shiftId) => {
              const k = `${emp.num}-${shiftId}`;
              ...
              next[k] = {
                ...existing,
                clockIn: machine.clockIn,
                clockOut: machine.clockOut,
                absent: false,
                _dirty: false,
              };
            });
      ...
      .catch((e) => {
        if (!cancelled) {
          setApiError(`Face-recognition attendance: ${String(e)}`);
        }
      })
      .finally(() => {
  ...
  }, [currentDate, employees, assignedShifts]);
...
const getEntry = (empNum: number): EmployeeTimeEntry => {
  const stored = entries[entryKey(empNum)] ?? {
    clockIn: "00:00",
    clockOut: "00:00",
    absent: false,
    absentComment: "",
  ...
  const machine = deviceEntries[String(empNum)];
  ...
  // Clock In / Clock Out are supplied by the face-recognition machine.
                      ...
                      {!entry.absent ? (
                        <div className="flex flex-col gap-1">
                          <span className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium ${
                            ...
                            <ClockInIcon />
                            {entry.clockIn !== "00:00" ? entry.clockIn : "No punch"}
                          </span>
                          <span className="text-[10px] text-gray-400">Face recognition</span>
                        </div>
                      ...
                      {!entry.absent ? (
                        <div className="flex flex-col gap-1">
                          <span className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium ${
                            ...
                            <ClockInIcon />
                            {entry.clockOut !== "00:00" ? entry.clockOut : "No punch"}
                          </span>
                          <span className="text-[10px] text-gray-400">Face recognition</span>
                        </div>
                      ) : <span className="text-xs text-gray-400 italic">—</span>}
